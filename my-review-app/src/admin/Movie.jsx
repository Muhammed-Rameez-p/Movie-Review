...
import { deleteMovie, getMovies, getMoviesBySearch } from '../actions/movie';
import { Link, useNavigate } from 'react-router-dom';


  
  export default function Movie({setCurrentMovieId}) {
...
<div className=" sm:flex  sm:items-end justify-between sticky top-0  bg-white bg-opacity-80 gap-x-6">
      ...
      <div>
      <Link to="/add-movie" >
        <button className="px-4 py-1.5 text-black bg-gray-500 border-l rounded mr-10 hover:bg-gray-300">Add Movie</button>
        </Link>
    </div>
        </div>
        ...
        { movie.map((person) => (
          <li key={person.title} className="flex justify-between gap-x-6 py-5">
            ...
            <div className="hidden sm:flex  sm:items-end">
             <button onClick={()=>{dispatch(deleteMovie(person._id))}} className="mr-3 inline-flex items-center rounded-md bg-red-50 px-2 py-1 
